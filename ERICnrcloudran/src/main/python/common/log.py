...
class NrCranLogger(object):
    """ This class abstracts all the logging and standard output printing
    methods used for NR-CRAN.
    """

    logger = None
    format = 'NR-CRAN %(levelname)s: %(message)s'
    ...
    def setup_log(cls):
        ...
        """
        if NrCranLogger.logger is not None:
            # setup only once...
            return
        NrCranLogger.logger = getLogger(__name__)
    ...
    def create_handlers(cls):
        ...
        log_path = os.path.join(
            os.path.dirname(os.path.abspath(__file__)), "../log/nrcran_log")
    ...
    def setup_handler(cls, handler, formatter):
        """
        Setups the handler with level and format.
        """
    ...
    def prints(cls, message, only_when_verbose=False):
        ...
        """
        if only_when_verbose and cls.settings["verbose"]\
                or not only_when_verbose:
